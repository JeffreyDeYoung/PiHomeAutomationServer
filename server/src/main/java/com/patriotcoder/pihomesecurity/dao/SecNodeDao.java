...
public class SecNodeDao {
   ...
   */
  public List<SecNode> getRunningNodes()
      throws IOException, IndexParseException, ParseException, RESTException {
    Query q = new Query();
   ...
   * @throws RESTException
   */
  public List<SecNode> getStoppedNodes()
      throws IOException, IndexParseException, ParseException, RESTException {
    Query q = new Query();
    q.setDatabase(Constants.DB);
    q.setTable(table.getName());
  ...
  public void saveNode(SecNode node)
      throws IOException, IndexParseException, ParseException, RESTException {
    ...
    } else// if it doesn't have an id, we don't know if it exists or not
    {
      ...
      if (nodeFetch != null)// if we get a response
      {
        ...
        SecNodeWithId toUpdate = (SecNodeWithId) node;// cast our node we are updating so we can set
                                                      // the id as well
  ...
  public SecNodeWithId getNodeByName(String nodeName)
      throws IOException, IndexParseException, ParseException, RESTException {
    Query q = new Query();
    q.setDatabase(Constants.DB);
