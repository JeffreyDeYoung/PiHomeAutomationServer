...
/**
 * Docussandra plugin that notifies nodes of relevent changes. (Must be placed in the home directory
 * of the Docussandra user.)
 *
...
public class NotifyNodes extends NotifierPlugin {
  ...
  public void doNotify(MutateType type, Document document) {
    try {
      if (type.equals(MutateType.CREATE) || type.equals(MutateType.UPDATE)) {
        if (document.getDatabaseName().equals(Constants.DB)
            && document.getTableName().equals(Constants.ACTOR_ABILITY_STATUS_TABLE)) {
          ServerSideAction action =
              (ServerSideAction) r.readValue(JSON.serialize(document.getObject()));
  ...
  private void doPostCall(String url, JSONObject toPost) throws Exception {
    ...
    RequestConfig rc = RequestConfig.custom().setConnectTimeout(5000).setSocketTimeout(5000)
    ...
    try {
      ...
      if (responseCode < 200 || responseCode >= 300) {
        throw new Exception("Error when doing a POST call agaist: " + url + ". JSON posted: "
            + toPost.toJSONString());
      }
